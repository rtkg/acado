																		...
																		)
{
	...
	process->getAll( LOG_DIFFERENTIAL_STATES,tmp );

	_diffStates.init( );
	
	for( uint i=0;i<tmp.getNumPoints()-1; ++i )
...
{
	MatrixVariablesGrid tmp;
	process->getAll( LOG_ALGEBRAIC_STATES,tmp );
...
{
	MatrixVariablesGrid tmp;
	process->getAll( LOG_INTERMEDIATE_STATES,tmp );
