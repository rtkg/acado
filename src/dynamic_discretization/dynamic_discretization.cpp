...
uint DynamicDiscretization::getNumEvaluationPoints() const{
    ...
    switch ( (PrintLevel) plotResolution ) {
         ...
         case LOW   : nEvaluationPoints = 10  / sqrt((double)nShoot) + 2; break;
         case MEDIUM: nEvaluationPoints = 30  / sqrt((double)nShoot) + 2; break;
         case HIGH  : nEvaluationPoints = 100 / sqrt((double)nShoot) + 2; break;
         default    : nEvaluationPoints = 10  / sqrt((double)nShoot) + 2; break;
    }
