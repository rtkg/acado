...
returnValue FunctionEvaluationTree::exportHeader(	FILE *file,
    ...
    if( getN() > 0 )
        acadoFPrintf(file,"       %s acado_aux[%d];\n", realString,nni );
...
returnValue FunctionEvaluationTree::exportCode(	FILE *file,
    ...
    if( getNU() > 0 ){
        acadoFPrintf(file,"%s *acado_u  = acado_x + %d;\n", realString,getNX()+getNXA() );
    }
    if( getNUI() > 0 ){
        acadoFPrintf(file,"%s *acado_v  = acado_x + %d;\n", realString,getNX()+getNXA()+getNU() );
    }
    if( getNP() > 0 ){
        acadoFPrintf(file,"%s *acado_p  = acado_x + %d;\n", realString,getNX()+getNXA()+getNU()+getNUI() );
    }
    if( getNPI() > 0 ){
        acadoFPrintf(file,"%s *acado_q  = acado_x + %d;\n", realString,getNX()+getNXA()+getNU()+getNUI()+getNP() );
    }
    if( getNW() > 0 ){
        acadoFPrintf(file,"%s *acado_w  = acado_x + %d;\n", realString,getNX()+getNXA()+getNU()+getNUI()+getNP()+getNPI() );
    }
    if( getNDX() > 0 ){
        acadoFPrintf(file,"%s *acado_dx = acado_x + %d;\n", realString,getNX()+getNXA()+getNU()+getNUI()+getNP()+getNPI()+getNW() );
    }
    acadoFPrintf(file,"\n");
    ...
    for( run1 = 0; run1 < n; run1++ ){
        acadoFPrintf(file,"acadoWorkspace.acado_aux[%d] = ", lhs_comp[run1] );
        file << *sub[run1];
        acadoFPrintf(file,";\n");
    }
