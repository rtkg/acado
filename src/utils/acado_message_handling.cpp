...
{ RET_RK45_CAN_NOT_TREAT_DAE,					"An explicit Runge-Kutta solver can not treat DAEs", VS_VISIBLE },
...
{ RET_ILLFORMED_HESSIAN_MATRIX,					"Hessian matrix is too ill-posed to continue", VS_VISIBLE },
...
{ RET_RELAXING_QP,								"QP needs to be relaxed due to numerical infeasibility", VS_VISIBLE },
...
returnValue::~returnValue() {
	if (data && (data->owner == this)) {
		...
		if (status != STATUS_HANDLED) print();
		
		delete data;
		if (level == LVL_FATAL) abort();
	}
}
