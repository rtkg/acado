...
%---------------------------------------------------------------------------------
%
%qpOASES_sequence is intended to solve a sequence of convex quadratic
%programmming (QP) problems of the following form:
%
%                min   0.5*x'Hx + x'g
%                s.t.  lb  <=  x <= ub
%                      lbA <= Ax <= ubA
...
%                    qpOASES_sequence( 'i',H,g,A,lb,ub,lbA,ubA,{nWSR,{x0}} )
...
%II) Call
%     [obj,x,y,status,nWSRout] = ...
%                     qpOASES_sequence( 'h',g,lb,ub,lbA,ubA,{nWSR} )
%for hotstarting from the previous QP solution to the one of the next QP
%given by the vectors g, lb, ub, lbA, ubA. If nWSR is not specified, a
...
%
%
%See also QPOASES_SEQUENCESB, QPOASES_SEQUENCEVM
%
%
