...
int int_qpOASES( char* fname )
{
	...
	CreateVar(  9,"d", &one,&one,&obj );
	CreateVar( 10,"d", &H_rows,&one,&x );
	CreateVar( 11,"d", &y_size,&one,&y );
	CreateVar( 12,"i", &one,&one,&status );
	CreateVar( 13,"i", &one,&one,&nWSRout );
	...
	if ( ( lb_rows != 0 ) && ( ub_rows != 0 ) )
	{
		qpoases(	stk(H),stk(g),stk(A),stk(lb),stk(ub),stk(lbA),stk(ubA),
					...
					stk(obj),stk(x),stk(y),istk(status),istk(nWSRout)
					);
	}
...
int int_hotstart( char* fname )
{
	...
	CreateVar(  7,"d", &one,&one,&obj );
	CreateVar(  8,"d", &qp_rowsH,&one,&x );
	CreateVar(  9,"d", &y_size,&one,&y );
	CreateVar( 10,"i", &one,&one,&status );
	CreateVar( 11,"i", &one,&one,&nWSRout );
...
int int_hotstartSB( char* fname )
{
	...
	CreateVar( 5,"d", &one,&one,&obj );
	CreateVar( 6,"d", &qpb_rowsH,&one,&x );
	CreateVar( 7,"d", &qpb_rowsH,&one,&y );
	CreateVar( 8,"i", &one,&one,&status );
	CreateVar( 9,"i", &one,&one,&nWSRout );
