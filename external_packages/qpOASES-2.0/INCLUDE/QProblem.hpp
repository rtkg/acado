...
class QProblem : public QProblemB
{
	...
	public:
		...
		returnValue init(	const double* const _H, 	/**< Hessian matrix. \n
							...
							double* const cputime		/**< Input: Maximum CPU time allowed for QP initialisation. \n
		...
		returnValue init(	const double* const _H, 					/**< Hessian matrix. \n
							...
							double* const cputime,						/**< Input: Maximum CPU time allowed for QP initialisation. \n
																			 Output: CPU time spend for QP initialisation. */
							const double* const xOpt,					/**< Optimal primal solution vector. \n
																			 (If a null pointer is passed, the old primal solution is kept!) */
							const double* const yOpt,					/**< Optimal dual solution vector. \n
		...
		returnValue hotstart(	const double* const g_new,		/**< Gradient of neighbouring QP to be solved. */
								const double* const lb_new,		/**< Lower bounds of neighbouring QP to be solved. \n
													 			 	 If no lower bounds exist, a NULL pointer can be passed. */
								const double* const ub_new,		/**< Upper bounds of neighbouring QP to be solved. \n
													 			 	 If no upper bounds exist, a NULL pointer can be passed. */
								const double* const lbA_new,	/**< Lower constraints' bounds of neighbouring QP to be solved. \n
													 			 	 If no lower constraints' bounds exist, a NULL pointer can be passed. */
								const double* const ubA_new,	/**< Upper constraints' bounds of neighbouring QP to be solved. \n
													 			 	 If no upper constraints' bounds exist, a NULL pointer can be passed. */
								int& nWSR,						/**< Input: Maximum number of working set recalculations; \n
															 		 Output: Number of performed working set recalculations. */
								double* const cputime			/**< Input: Maximum CPU time allowed for QP solution. \n
		...
		inline returnValue getA(	double* const _A	/**< Array of appropriate dimension for copying constraint matrix.*/
									) const;
		...
		inline returnValue getA(	int number,			/**< Number of entry to be returned. */
									double* const row	/**< Array of appropriate dimension for copying (number)th constraint. */
									) const;

		/** Returns lower constraints' bound vector of the QP (deep copy).
		  *	\return SUCCESSFUL_RETURN \n
		  			RET_QPOBJECT_NOT_SETUP */
		inline returnValue getLBA(	double* const _lbA	/**< Array of appropriate dimension for copying lower constraints' bound vector.*/
									) const;
		...
		inline returnValue getLBA(	int number,		/**< Number of entry to be returned. */
									double& value	/**< Output: lbA[number].*/
									) const;

		/** Returns upper constraints' bound vector of the QP (deep copy).
		  *	\return SUCCESSFUL_RETURN \n
		  			RET_QPOBJECT_NOT_SETUP */
		inline returnValue getUBA(	double* const _ubA	/**< Array of appropriate dimension for copying upper constraints' bound vector.*/
									) const;
		...
		inline returnValue getUBA(	int number,		/**< Number of entry to be returned. */
									double& value	/**< Output: ubA[number].*/
									) const;
	...
	protected:
		 ...
		 *	\return SUCCESSFUL_RETURN \n
					...
					RET_INIT_FAILED_UNBOUNDEDNESS \n
					RET_MAX_NWSR_REACHED */
		returnValue solveInitialQP(	const double* const xOpt,						/**< Optimal primal solution vector.
		 ...
		 *	\return SUCCESSFUL_RETURN \n
					...
					RET_HOTSTART_STOPPED_INFEASIBILITY \n
					RET_HOTSTART_STOPPED_UNBOUNDEDNESS */
		returnValue solveQP(	const double* const g_new,		/**< Gradient of neighbouring QP to be solved. */
								const double* const lb_new,	/**< Lower bounds of neighbouring QP to be solved. \n
													 			 	 If no lower bounds exist, a NULL pointer can be passed. */
								const double* const ub_new,		/**< Upper bounds of neighbouring QP to be solved. \n
													 			 	 If no upper bounds exist, a NULL pointer can be passed. */
		 ...
		 *	\return SUCCESSFUL_RETURN \n
					...
					RET_HOTSTART_STOPPED_INFEASIBILITY \n
					RET_HOTSTART_STOPPED_UNBOUNDEDNESS */
		returnValue solveRegularisedQP(	const double* const g_new,		/**< Gradient of neighbouring QP to be solved. */
										const double* const lb_new,		/**< Lower bounds of neighbouring QP to be solved. \n
															 			 	 If no lower bounds exist, a NULL pointer can be passed. */
										const double* const ub_new,		/**< Upper bounds of neighbouring QP to be solved. \n
															 			 	 If no upper bounds exist, a NULL pointer can be passed. */
		 ...
		 *	\return SUCCESSFUL_RETURN \n
					RET_SETUPSUBJECTTOTYPE_FAILED */
		virtual returnValue setupSubjectToType(	const double* const lb_new,		/**< New lower bounds. */
												const double* const ub_new,		/**< New upper bounds. */
												const double* const lbA_new,	/**< New lower constraints' bounds. */
												const double* const ubA_new		/**< New upper constraints' bounds. */
												);
		 ...
		 *	\return SUCCESSFUL_RETURN \n
					...
					RET_INVALID_ARGUMENTS */
		returnValue obtainAuxiliaryWorkingSet(	const double* const xOpt,						/**< Optimal primal solution vector.
		 ...
		 *	\return SUCCESSFUL_RETURN */
		returnValue setupAuxiliaryQPsolution(	const double* const xOpt,			/**< Optimal primal solution vector.
		 ...
		 *	\return SUCCESSFUL_RETURN \n
		 			RET_DIV_BY_ZERO */
		returnValue backsolveT(	const double* const b,	/**< Right hand side vector. */
								BooleanType transposed,	/**< Indicates if the transposed system shall be solved. */
								double* const a 		/**< Output: Solution vector */
								) const;
		...
		returnValue determineDataShift(	const int* const FX_idx, 	/**< Index array of fixed variables. */
										...
										const double* const g_new,	/**< New gradient vector. */
										const double* const lbA_new,/**< New lower constraints' bounds. */
										const double* const ubA_new,/**< New upper constraints' bounds. */
										const double* const lb_new,	/**< New lower bounds. */
										const double* const ub_new,	/**< New upper bounds. */
										double* const delta_g,	 	/**< Output: Step direction of gradient vector. */
										double* const delta_lbA,	/**< Output: Step direction of lower constraints' bounds. */
										double* const delta_ubA,	/**< Output: Step direction of upper constraints' bounds. */
										double* const delta_lb,	 	/**< Output: Step direction of lower bounds. */
										double* const delta_ub,	 	/**< Output: Step direction of upper bounds. */
		...
		returnValue determineStepDirection(	const int* const FR_idx, 		/**< Index array of free variables. */
											...
											const int* const AC_idx, 		/**< Index array of active constraints. */
											const double* const delta_g,	/**< Step direction of gradient vector. */
											const double* const delta_lbA,	/**< Step direction of lower constraints' bounds. */
											const double* const delta_ubA,	/**< Step direction of upper constraints' bounds. */
											const double* const delta_lb,	/**< Step direction of lower bounds. */
											const double* const delta_ub,	/**< Step direction of upper bounds. */
											...
											double* const delta_xFX, 		/**< Output: Primal homotopy step direction of fixed variables. */
											double* const delta_xFR,	 	/**< Output: Primal homotopy step direction of free variables. */
											double* const delta_yAC, 		/**< Output: Dual homotopy step direction of active constraints' multiplier. */
											double* const delta_yFX 		/**< Output: Dual homotopy step direction of fixed variables' multiplier. */
											) const;
		...
		returnValue determineStepLength(	const int* const FR_idx, 			/**< Index array of free variables. */
											...
											const double* const delta_lb,	 	/**< Step direction of lower bounds. */
											const double* const delta_ub,	 	/**< Step direction of upper bounds. */
											const double* const delta_xFX, 		/**< Primal homotopy step direction of fixed variables. */
											const double* const delta_xFR,		/**< Primal homotopy step direction of free variables. */
											const double* const delta_yAC,		/**< Dual homotopy step direction of active constraints' multiplier. */
											const double* const delta_yFX,		/**< Dual homotopy step direction of fixed variables' multiplier. */
											double* const delta_Ax_l,			/**< Output: Step in vector Ax-lbA. */
											double* const delta_Ax_u,			/**< Output: Step in vector ubA-Ax. */
		...
		returnValue performStep(	const int* const FR_idx, 			/**< Index array of free variables. */
									...
									const double* const delta_yAC,	 	/**< Dual homotopy step direction of active constraints' multiplier. */
									const double* const delta_yFX, 		/**< Dual homotopy step direction of fixed variables' multiplier. */
									const double* const delta_Ax_l, 	/**< Step in vector Ax-lbA. */
									const double* const delta_Ax_u,		/**< Step in vector ubA-Ax. */
		 ...
		 */
		BooleanType areBoundsConsistent(	const double* const delta_lb,	/**< Step direction of lower bounds. */
											const double* const delta_ub,	/**< Step direction of upper bounds. */
											const double* const delta_lbA,	/**< Step direction of lower constraints' bounds. */
											const double* const delta_ubA	/**< Step direction of upper constraints' bounds. */
											) const;
		 ...
		 *	\return SUCCESSFUL_RETURN \n
					RET_INVALID_ARGUMENTS \n
					RET_UNKNONW_BUG */
		returnValue setupQPdata(	const double* const _H, 	/**< Hessian matrix. \n
																     If Hessian matrix is trivial,a NULL pointer can be passed. */
									const double* const _g, 	/**< Gradient vector. */
									const double* const _A,  	/**< Constraint matrix. */
									const double* const _lb,	/**< Lower bound vector (on variables). \n
		...
		returnValue loadQPvectorsFromFile(	const char* const g_file, 	/**< Name of file where gradient, of neighbouring QP to be solved, is stored. */
											...
											const char* const ubA_file, /**< Name of file where upper constraints' bounds, of neighbouring QP to be solved, is stored. \n
												 			 				 If no upper constraints' bounds exist, a NULL pointer can be passed. */
											double* const g_new,		/**< Output: Gradient of neighbouring QP to be solved. */
											double* const lb_new,		/**< Output: Lower bounds of neighbouring QP to be solved */
											double* const ub_new,		/**< Output: Upper bounds of neighbouring QP to be solved */
											double* const lbA_new,		/**< Output: Lower constraints' bounds of neighbouring QP to be solved */
											double* const ubA_new		/**< Output: Upper constraints' bounds of neighbouring QP to be solved */
											) const;
		 ...
		 *	\return SUCCESSFUL_RETURN \n
		 *			RET_INVALID_ARGUMENTS */
		inline returnValue setA(	const double* const A_new	/**< New constraint matrix (with correct dimension!). */
									);
		...
		inline returnValue setA(	int number,					/**< Number of row to be changed. */
									const double* const value	/**< New (number)th constraint (with correct dimension!). */
									);
		 ...
		 *	\return SUCCESSFUL_RETURN \n
		 *			RET_INVALID_ARGUMENTS */
		inline returnValue setLBA(	const double* const lbA_new	/**< New constraints' lower bound vector (with correct dimension!). */
									);
		...
		inline returnValue setLBA(	int number,		/**< Number of entry to be changed. */
									double value	/**< New value for entry of lower constraints' bound vector (with correct dimension!). */
									);

		/** Sets constraints' upper bound vector of the QP.
		 *	\return SUCCESSFUL_RETURN \n
		 *			RET_INVALID_ARGUMENTS */
		inline returnValue setUBA(	const double* const ubA_new	/**< New constraints' upper bound vector (with correct dimension!). */
									);
		...
		inline returnValue setUBA(	int number,		/**< Number of entry to be changed. */
									double value	/**< New value for entry of upper constraints' bound vector (with correct dimension!). */
									);
	...
	protected:
		...
		double* lbA;							/**< Lower constraints' bound vector. */
		double* ubA;							/**< Upper constraints' bound vector. */

		Constraints* constraints;				/**< Data structure for problem's constraints. */

		double* T;								/**< Reverse triangular matrix, A = [0 T]*Q'. */
		double* Q;								/**< Orthonormal quadratic matrix, A = [0 T]*Q'. */
		int sizeT;								/**< Matrix T is stored in a (sizeT x sizeT) array. */

		double* Ax_l;							/**< Stores the current distance to lower constraints' bounds A*x-lbA \n
												 *	 (for increased efficiency only). */
		double* Ax_u;							/**< Stores the current distance to lower constraints' bounds ubA-A*x \n
												 *	 (for increased efficiency only). */
