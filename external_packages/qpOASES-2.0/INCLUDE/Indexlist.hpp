...
class Indexlist
{
	...
	public:
		...
		/** Add all indices from another index set.
		 *	\return SUCCESSFUL_RETURN */
		returnValue add(	const Indexlist* const IS	/**< Index set whose indices are to be added. */
							);

		/** Intersects the index set with another index set IS
		 *	(i.e. all indices not contained in IS are removed).
		 *	\return SUCCESSFUL_RETURN */
		returnValue intersect(	const Indexlist* const IS	/**< Index set IS for intersection. */
								);

		/** Deletes all indices that are also contained in another index set IS.
		 *	\return SUCCESSFUL_RETURN */
		returnValue setminus(	const Indexlist* const IS	/**< Index set IS for deleting indices. */
								);
	...
	protected:
		...
		int* next;			/**< Array to store physical index of successor. */
		int* previous;		/**< Array to store physical index of predecossor. */
