...
/**
 *    \file external_packages/gnuplot/acado2gnuplot.cpp
 *    \author Boris Houska, Hans Joachim Ferreau
 *    \date   2009
 */


#include "acado2gnuplot.hpp"
...
returnValue GnuplotWindow::sendDataToGnuplot( )
{
	...
	VariablesGrid discretizationGrid;
			...
			if( run1 < getNumSubplots( ) )
			{
				...
				switch ( operator()(run1).getSubPlotType( ) )
				{
					...
					case SPT_VARIABLES_GRID:
						...
						dataGrid = *(operator()(run1).plotVariablesGrid);
						break;
...
// 				}
				...
				if( userDataString != 0 )
					acadoFPrintf(gnuPipe,",\\\n'-' using 1:2 title '' with lines lt 1 lw 3\n" );
				else
					acadoFPrintf(gnuPipe,"\n");
												...
												) const
{
	switch( _type )
	{
		...
		case VT_PARAMETER:
			return acadoAssignString( plotStyleString,"lt 7","lt 7" ); //orange
			break;
